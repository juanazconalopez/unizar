...
export async function fetchTrainingData(userId: string): Promise<TrainingData> {
  const { data: ownProfile, error: profileError } = await supabase
    .from('profiles')
    .select('id, display_name, is_approved, is_active, is_collaborator, is_owner, created_at')
    .eq('id', userId)
    .single()

  if (profileError) throw profileError
  const profile = ownProfile as Profile
  const emptyData = { profile, seasons: [], memberships: [], profiles: [], tasks: [], results: [] }
  if (!profile.is_approved) return emptyData

  const [seasonsResponse, tasksResponse, resultsResponse, membershipsResponse] = await Promise.all([
