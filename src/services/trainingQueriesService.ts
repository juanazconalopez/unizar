...
export async function fetchHomeAttention(today: string, seasonEnd: string) {
  ...
  const [matchesResponse, announcementsResponse] = await Promise.all([
    supabase.from('matches').select('*, seasons(name), season_competitions(id,name,color,is_default), season_teams(id,name,is_mixed,is_default)').eq('status', 'published').gte('match_date', today).order('match_date', { ascending: true }).limit(1),
...
export async function fetchMatchWindow(fromDate: string, toDate?: string): Promise<MatchWindowData> {
  let query = supabase.from('matches').select('*, seasons(name), season_competitions(id,name,color,is_default), season_teams(id,name,is_mixed,is_default)').gte('match_date', fromDate).order('match_date', { ascending: true })
