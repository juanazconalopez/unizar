...
export function TrainingPlanDetail({ plan, onBack, onDuplicate, onEdit }: { plan: TrainingPlan; onBack: () => void; onDuplicate?: () => void; onEdit?: () => void }) {
  ...
  return <div className="page training-detail-page">
    ...
    <header className="training-detail-hero">
      ...
      <div className="training-detail-hero-actions"><div className="training-detail-duration"><strong>{totalDuration}</strong><span>minutos</span></div><button className="secondary-button training-pdf-button" onClick={() => window.print()} type="button"><Icon name="download" size={16} />Guardar PDF</button>{(onDuplicate || onEdit) && <div className="training-detail-quick-actions">{onDuplicate && <button aria-label="Duplicar entrenamiento" className="icon-button" onClick={onDuplicate} title="Duplicar entrenamiento" type="button"><Icon name="copy" size={17} /></button>}{onEdit && <button aria-label="Editar entrenamiento" className="icon-button" onClick={onEdit} title="Editar entrenamiento" type="button"><Icon name="edit" size={17} /></button>}</div>}</div>
    </header>
    <section className="training-detail-overview">
      ...
      {plan.material && <article><span className="eyebrow">MATERIAL</span><RichContent eagerImages text={plan.material} /></article>}
