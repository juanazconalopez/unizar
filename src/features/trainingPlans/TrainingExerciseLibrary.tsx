      ...
      : loading ? <div className="training-loading">Cargando biblioteca…</div>
        : filtered.length ? <div className="training-library-grid">{filtered.map((preset) => {
          ...
          return <article className="training-library-card" key={preset.id}>
            {onEdit && <button aria-label={`Editar ejercicio ${preset.title}`} className="training-library-card-open" onClick={() => onEdit(preset)} type="button" />}
            ...
            <div className="training-library-card-content"><div className="training-library-card-meta"><span><Icon name="clock" size={14} />{preset.duration_minutes} min</span>{hasDiagram && <span className="has-diagram"><Icon name="strategy" size={14} />Con esquema</span>}{contentImageIds(preset.description).length > 0 && <span>📎 Imagen</span>}</div><h2>{preset.title}</h2><div className="training-library-card-description"><RichContent fallback="Sin descripción." renderImages={false} text={preset.description} /></div></div>
            {onEdit && <button className="secondary-button compact training-library-edit" onClick={() => onEdit(preset)} type="button">Editar</button>}
          </article>
        })}</div>
