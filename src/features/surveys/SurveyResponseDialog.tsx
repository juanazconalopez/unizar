...
export function SurveyResponseDialog({ surveyId, onClose, onDone }: { surveyId: string; onClose: () => void; onDone: () => Promise<void> }) {
  const [survey, setSurvey] = useState<Awaited<ReturnType<typeof fetchSurveyForResponse>> | null>(null)
  const [error, setError] = useState('')
  useEffect(() => { let active = true; void fetchSurveyForResponse(surveyId).then((value) => { if (active) setSurvey(value) }).catch((cause) => { if (active) setError(cause instanceof Error ? cause.message : 'No se pudo cargar la encuesta.') }); return () => { active = false } }, [surveyId])
