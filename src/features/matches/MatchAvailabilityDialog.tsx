...
const groups: { status: AvailabilityStatus; title: string; empty: string }[] = [
...
export function MatchAvailabilityDialog({ availability, match, profiles, onClose }: {
  ...
  return <Modal className="availability-details-dialog" labelledBy={titleId} onClose={onClose}>
    ...
    <div className="availability-groups">{groups.map((group) => {
      const responses = availability.filter((item) => item.status === group.status)
