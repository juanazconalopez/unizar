...
function MatchCard({ availability, isOwner, lineup, match, ownAvailability, onEdit, onManageLineup, onSaveAvailability, onViewLineup }: { availability: MatchAvailability[]; isOwner: boolean; lineup: MatchLineup[]; match: Match; ownAvailability?: MatchAvailability; onEdit: () => void; onManageLineup: () => void; onSaveAvailability: (match: Match, status: AvailabilityStatus, comment: string) => Promise<void>; onViewLineup: () => void }) {
  return <article className="match-card"><div className="match-card-heading"><div><span className="eyebrow">{match.is_home ? 'LOCAL' : 'VISITANTE'} · {match.match_kind === 'official' ? 'OFICIAL' : 'AMISTOSO'} · {match.rugby_format === 'sevens' ? 'SEVEN' : 'XV'}</span><h2>{match.is_home ? <>CDU Rugby <i>vs</i> {match.opponent}</> : <>{match.opponent} <i>vs</i> CDU Rugby</>}</h2><p>{formatDate(match.match_date, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}{match.kickoff_time ? ` · ${match.kickoff_time.slice(0, 5)}` : ''}{match.venue ? ` · ${match.venue}` : ''}</p></div><span className={`match-status ${match.status}`}>{matchStatus(match.status)}</span></div>{match.notes && <p className="match-notes">{match.notes}</p>}
    {isOwner ? <><AvailabilitySummary availability={availability} /><div className="match-actions"><button className="secondary-button compact" onClick={onEdit}>Editar partido</button>{match.lineup_published && lineup.length > 0 && <button className="secondary-button compact" onClick={onViewLineup}>Ver convocatoria</button>}<button className="primary-button compact" onClick={onManageLineup}>Gestionar alineación</button></div></> : <><MatchAvailabilityResponse initial={ownAvailability} match={match} onSave={onSaveAvailability} />{match.lineup_published && lineup.length > 0 && <div className="match-actions"><button className="secondary-button compact" onClick={onViewLineup}>Ver convocatoria</button></div>}</>}
  </article>
}

function AvailabilitySummary({ availability }: { availability: MatchAvailability[] }) { const count = (status: AvailabilityStatus) => availability.filter((item) => item.status === status).length; return <div className="availability-summary"><span className="available">{count('available')} disponibles</span><span className="doubt">{count('doubt')} dudas</span><span className="unavailable">{count('unavailable')} no disponibles</span></div> }
