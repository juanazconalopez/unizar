...
test('match availability and lineup flows work on mobile', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: /Unizar Fem/ })).toBeVisible()
...
test('Dirección can inspect team data but cannot edit it', async ({ page }) => {
  ...
  await page.locator('.match-card-summary').first().click()
  await expect(page.getByRole('button', { name: 'Ver disponibilidades' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Editar partido' })).toHaveCount(0)
  await expect(page.getByRole('button', { name: 'Preparar convocatoria' })).toHaveCount(0)
